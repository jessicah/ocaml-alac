...
let sign_of_int i =
	(*let negishift = (-i) lsr 31 in
	negishift lor (i lsr 31)*)
	let negishift = Int32.shift_right_logical (Int32.neg i) 31 in
	Int32.logor negishift (Int32.shift_right_logical i 31)
...
let unpc_block (pc1 : int32a) (out : int32a) num (coefs : int16a) numactive chanbits denshift =
	...
	if numactive = 0 then begin
		...
		for j = 1 to num - 1 do
			...
			prev := Int32.shift_right_logical (Int32.shift_left del chanshift) chanshift;
			out.{j} <- !prev;
		done
	end else begin
		for j = 1 to numactive do
			let del = Int32.add pc1.{j} out.{j-1} in
			out.{j} <- Int32.shift_right_logical (Int32.shift_left del chanshift) chanshift;
		done;
		...
		for j = lim to num - 1 do
			...
			let del = Int32.add del (Int32.add top (Int32.shift_right_logical (Int32.add !sum1 (Int32.of_int denhalf)) denshift)) in
			out.{j} <- Int32.shift_right_logical (Int32.shift_left del chanshift) chanshift;
			...
			if sg > 0l then begin
				loop_while (fun _ -> !del0 > 0l) (fun k ->
					...
					del0 := Int32.sub !del0 (Int32.mul (Int32.of_int (numactive - k)) (Int32.shift_right_logical (Int32.mul sgn dd) denshift));
					) (numactive - 1);
				...
				loop_while (fun _ -> !del0 < 0l) (fun k ->
					...
					let sgn = sign_of_int dd in
					coefs.{k} <- coefs.{k} + (Int32.to_int sgn);
					del0 := Int32.sub !del0 (Int32.mul (Int32.of_int (numactive - k)) (Int32.shift_right_logical (Int32.mul (Int32.neg sgn) dd) denshift));
					) (numactive - 1);
			end;
		done;
	end
